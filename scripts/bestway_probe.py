...
from homeflow.integrations.bestway.client import BestwayClient  # noqa: E402
from homeflow.integrations.bestway.datapoints import (  # noqa: E402
    BitLocation,
    Datapoint,
    DatapointProfile,
    ProfileError,
...
def differences(previous: bytes, current: bytes) -> list[str]:
    ...
    for offset in range(max(len(previous), len(current))):
        ...
        changes.append(f"  byte {offset}: {before:3d} -> {after:3d}  ({before:08b} -> {after:08b}){bits}")
    return changes
...
async def probe(args: argparse.Namespace) -> int:
    ...
    print(f"verified    : {'yes' if profile.trusted else 'NO - nothing is exposed or writable yet'}")
...
def main() -> int:
    ...
    parser.add_argument("--profile", default="airjet-candidate", help="built-in layout to decode with")
